# like sample_multiclass_classification_data.txt

import random
import sys


def generate(fileName, columnNum, lineCnt, typeNum, classNum):
    with open(fileName, 'w') as fi:
        for i in range(lineCnt):
            cl = random.randint(0, classNum - 1)
            line = "{0} ".format(cl)
...
classNum = int(sys.argv[5]) if len(sys.argv) >= 6 else 10
generate(out, columnNum, lineCnt, typeNum, classNum)
