...
def gen_data(data_path, hdfs_path):
    lineCnt = randint(7000000, 10000000)
    columnCnt = randint(5, 30)
    typeNum = randint(10, 200)
    classNum = randint(5, 21)

    # 1.1 gen local data
    os.system("rm -f {0}".format(data_path))
    print 'generating testData, path: ', data_path
    os.system("python For_multi_class.py {0} {1} {2} {3}"
              .format(data_path, columnCnt, lineCnt, typeNum, classNum))
    # 1.2 copy to hdfs
...
def run_jar(hdfs_path):
    ...
    cmd = '/home/spark-2.1.0-bin-hadoop2.6/bin/spark-submit --class JavaMultilayerPerceptronClassifierExample ' \
        ...
        .format(randint(1, 6), driver_mem, exe_mem, exe_cores, hdfs_path, randint(17, 71))
...
if __name__ == "__main__":
    ...
    data_path = "/home/testData/multi11.txt"
    hdfs_path = "/testData/multi11.txt"
