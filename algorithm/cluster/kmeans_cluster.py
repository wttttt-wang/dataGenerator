...
class KMeansClassifier:
    ...
    def __init__(self, k=3, initCent='random', max_iter=500, dis_metrics='edist'):
    ...
    def _calDtwDict(self, arrA, arrB):
        ser1_norm = arrA.reshape(-1, 1)
        ser2_norm = arrB.reshape(-1, 1)
        ...
        for _ in range(self._max_iter):
            ...
            for i in range(m):  # 将每个样本点分配到离它最近的质心所属的族
                ...
                for j in range(self._k):  # 次迭代用于寻找最近的质心
                    ...
                    arrB = data_X[i, :]
                    if self._dis_metrics == 'dtwdist':
                        distJI = self._calDtwDict(arrA, arrB)
                    else:
    ...
    def predict(self, X):  # 根据聚类结果，预测新输入数据所属的族
        ...
        for i in range(m):  # 将每个样本点分配到离它最近的质心所属的族
            ...
            for j in range(self._k):
                if self._dis_metrics == 'dtwdist':
                    distJI = self._calDtwDict(self._centroids[j, :], X[i, :])
                else:
